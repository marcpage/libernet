...
int main(const int /*argc*/, const char *const /*argv*/[]) {
  ...
  try {
    ...
    for (int i = 0; i < iterations; ++i) {
    	...
    	p1.addCredential(dummyData.identifer(), dummyData.key(), "dummy");
    	p1.addCredential(yummyData.identifer(), yummyData.key(), "yummy");
    	...
    	dotest(p1.credentialIdentifier(0) == dummyData.identifer());
		dotest(p1.credentialKey(dummyData.identifer()) == dummyData.key());
		dotest(p1.credentialFilename(dummyData.identifer()) == "dummy");

    	dotest(p1.credentialIdentifier(1) == yummyData.identifer());
		dotest(p1.credentialKey(yummyData.identifer()) == yummyData.key());
		dotest(p1.credentialFilename(yummyData.identifer()) == "yummy");
