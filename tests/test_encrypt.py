...
def test_RSA_Identity():
    ...
    restored_identity = libernet.tools.encrypt.RSA_Identity(json.loads(serialized_private))
    ...
    assert signature == restored_identity.sign(libernet.tools.hash.sha256_hasher(b'to sign'))
    assert restored_public.verify(libernet.tools.hash.sha256_hasher(b'to sign'), signature)
    assert not restored_public.verify(libernet.tools.hash.sha256_hasher(b'not signed'), signature)
