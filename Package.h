...
namespace pkg {
	...
	*/
	inline void assertIdentifier(const std::string &nameType, const std::string &/*name*/, const std::string &keyType, const std::string &key) {
		if (nameType != "sha256") {
	...
	/** Takes a resource identifier and breaks it down into its parts.
		@param identifier The identifier of the form [HASH_TYPE]:[HASH]:[KEY_TYPE]:[KEY] or [HASH_TYPE]:[HASH]
		@param nameType [OUTPUT] The type of data identifier (HASH_TYPE). May return empty if invalid form
		@param name [OUTPUT] The name of the data (HASH). May return empty if invalid form
		@param keyType [OUTPUT] The type of key to decrypt the data (KEY_TYPE).
						May return empty if no encryption was specified or invalid form
	...
	*/
		...
		if (std::string::npos != firstColon) {
			...
			if (std::string::npos != secondColon) {
				...
				if (std::string::npos != thirdColon) {
					keyType.assign(identifier, secondColon + 1, thirdColon - secondColon - 1);
	...
	/** Optionally compresses and encrypts data, returning identifiers for the data.
		...
		@return The full identifier of the data, used to request and decrypt the data
	*/
	...
	/** Given a block of data, pack it, store it, and return its full identifier to get the data back.
		Data will not be written if it is already in the storagePath.
	...
	*/
	...
	/** Encrypt and store the contents of a file and return the file identifier.
		Files in broken up into 1 MiB chunks, each separately compressed (optionally),
		encrypted, and stored. A resulting identifier that can identifer and restore the
	...
	*/
	...
	/** Given the path to a directory, compresses, encrypts, and stores the data parts.
	...
	*/
	...
	/** Package a directory and store all data necessary to retrieve and restore the directory.
		@param path The directory to store
		@param storagePath The path to a directory to store data parts in
	...
	*/
	...
	inline std::string unpackData(const std::string &packedData, const std::string &identifier) {
		...
		if (hash::sha256(uncompressedData).hex() != finalDataHash) {
			throw json::WrongType("Name does not match contents: expected='" + finalDataHash
									+ "' hash='" + hash::sha256(uncompressedData).hex() + "'", __FILE__, __LINE__);
		}
