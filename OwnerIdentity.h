...
public:
  ...
  OwnedIdentity(const io::Path &file, const std::string &passphrase):_key() {
  	_decrypt(data::Data(file.contents(io::File::Binary), data::Data::Unencrypted));
  }
  OwnedIdentity(const Data &data, const std::string &passphrase):Identity(), _key() {
  	_decrypt(data, passphrase);
  }
  OwnedIdentity(const OwnedIdentity &other):Identity(other), _key(other._key) {}
  virtual ~OwnedIdentity() {}
  Identity &operator=(const OwnedIdentity &other) {
    Identiter::operator=(other);
    _key = other._key();
  }
  data::Data ownedValue(const std::string &passphrase) { return _encrypt(passphrase); }
  ...
  void writeOwned(const io::Path &file, const std::string &passphrase) {
  	file.write(_encrypt(passphrase).contents(), io::File::Binary);
  }

private:
  crypt::RSAAES256PrivateKey _key;
  static unsigned long _randomPublicExponent();
  data::Data _encrypt(const std::string &passphrase);
  void _decrypt(data::Data &data, const std::string &passphrase);
  static int _matching(const std::string &s1, const std::string s2);
};
...
inline data::Data OwnedIdentity::_encrypt(const std::string &passphrase) {
    ...
    crypto::AES256 key(reinterpret_cast<const char *>(keyData.buffer()),
                       keyData.size());
	std::string match("passphrase:" + passphrase).hex();
...
inline void OwnedIdentity::_decrypt(data::Data &data, const std::string &passphrase) {
	json::Value info;
	std::string contents;
	std::string	buffer;
	data::Data output;
	...
	key.crypto::SymmetricKey::decryptInPlace(data.contents(), "", contents);
	info.parse(contents);
	Identity::operator=(Identity(data::Data(info["public"], info["identifier"])));
	_key = RSAAES256PrivateKey(info["owner"]);
}

inline int OwnedIdentity::_matching(const std::string &s1, const std::string s2) {
